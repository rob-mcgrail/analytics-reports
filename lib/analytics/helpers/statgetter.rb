module StatGetters
  ...
  #mixins for various API classes
  ...
  def make_bounces_rates
      ...
      if @list_visits.length != @list_bounces.length
        raise "The two arrays you are using in foo.make_bounces_rates are not of the same length!"
      end
  ...
  def make_times_average_sessions
    ...
    if @list_visits.length != @list_times.length
      raise "The two arrays you are using in foo.make_times_average_sessions are not of the same length!"
    end
